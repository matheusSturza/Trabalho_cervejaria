...
class CervejaDAO {
    ...
    public function listar() {
        ...
        $pstmt->execute();
        return $this->montarLista($pstmt);
    }

    private function montarLista($pstmt){
        $vetor = [];
    ...
    public function listarPais(){
        $pstmt = $this->conexao->prepare("SELECT * FROM cerveja ORDER BY pais_origem ASC");
        $pstmt->execute();
        return $this->montarLista($pstmt);
    }

    public function listarNome(){
        $pstmt = $this->conexao->prepare("SELECT * FROM cerveja ORDER BY nome ASC");
        $pstmt->execute();
        return $this->montarLista($pstmt);
    }

    public function listarNota(){
        $pstmt = $this->conexao->prepare("SELECT * FROM cerveja ORDER BY avaliacao DESC");
        $pstmt->execute();
        return $this->montarLista($pstmt);
    }
    ...
    public function listarAvaliacao($aval){
    ...
    public function listarData($data){
        $pstmt = $this->conexao->prepare("SELECT * FROM cerveja WHERE data_fabri = :data");
        $pstmt->bindValue(":data", $data);
        $pstmt->execute();
        return $this->montarLista($pstmt);
    }
    ...
    public function listarImgs(){ 
        $lista = []; 
        $sql = "SELECT img FROM cerveja WHERE img IS NOT NULL"; 
